...
$publish  = mysqli_fetch_assoc(mysqli_query($conn,
            "SELECT COUNT(*) AS total FROM articles WHERE status='dipublikasikan'"))['total'] ?? 0;
...
$jurnalis = mysqli_fetch_assoc(mysqli_query($conn,
            "SELECT COUNT(*) AS total FROM users WHERE role='jurnalis' AND status='aktif'"))['total'] ?? 0;
...
<section class="card dashboard-section">
    ...
    <div class="stat-box">
        ...
        <div class="stat-card">
            <div class="value" id="stat-publish" data-value="<?= $publish ?>" style="color:#2563eb;">
                <?= $publish ?>
            </div>
        ...
        <div class="stat-card">
            <div class="value" id="stat-pending" data-value="<?= $pending ?>" style="color:#d97706;">
                <?= $pending ?>
            </div>
        ...
        <div class="stat-card">
            <div class="value" id="stat-jurnalis" data-value="<?= $jurnalis ?>" style="color:#047857;">
                <?= $jurnalis ?>
            </div>
        ...
        <div class="stat-card">
            <div class="value" id="stat-pembaca" data-value="<?= $pembacaRaw ?>" style="color:#7c3aed;">
                <?= $pembaca ?>
            </div>
    ...
    <div class="card stats-box">
        ...
        <div style="display:flex; gap:20px; width:100%; height:300px;">

            <!-- Grafik Artikel -->
            <div style="flex:1; background:white; padding:10px; border-radius:10px;">
                <canvas id="chartArtikel"></canvas>
            </div>

            <!-- Grafik Pembaca -->
            <div style="flex:1; background:white; padding:10px; border-radius:10px;">
                <canvas id="chartPembaca"></canvas>
            </div>
...
<!-- LOAD FILE JAVASCRIPT EKSTERNAL -->
